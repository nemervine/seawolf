...
class PathTwoEntity(VisionEntity):
    name = "PathTwo"

    def init(self):
        '''special init function required for VisionEntity object.'''
        ...
        self.seen_in_a_row_threshold = 2 # Must see path this many times in a\
    ...
    def prepare_frame(self, frame):
        '''Prepare the frame for analysis by reducing it to a set of edges.
        
        Notes on line format in polar form at:
        http://docs.opencv.org/doc/tutorials/imgproc/imgtrans/hough_lines/hough_lines.html
        '''
        cv.Smooth(frame, frame, cv.CV_MEDIAN, 7, 7)

        # Use RGB color finder.  The function is : find_target_color_rgb(
        # frame, r, g, b, min_blobsize,dev_threshold, precision_threshold)
        binary = libvision.cmodules.target_color_rgb.find_target_color_rgb\
                    (frame,250,125,0,1500,500,.3)

        if self.debug:
            self.color_filtered = cv.CloneImage(binary)
    ...
    def hough(self, binary_img):
        '''Take a binary image and perform a hough transform on  it.  Then
        return a list of the lines that were found.
        
        Keyword Arguments:
        binary_img -- a binary img to be processed
        Returns list of found lines
        '''
        line_storage = cv.CreateMemStorage()
        lines = cv.HoughLines2(binary_img, line_storage, cv.CV_HOUGH_STANDARD,
            rho=1,
            theta=math.pi/180,
        ...
        return lines[:self.lines_to_consider] # Limit number of lines
        

    def find_path(self, lines):
        '''Given a list of lines found, determine if a path has been found.
        The idea is that if there are at least two lines, and they are
        parallel, then we assume that there must be a path.
        ...
        if len(lines) >= 2:
            ...
            for rho, theta in lines: # Find: min, max
                total_theta += theta
                if theta_max < theta:
                    theta_max = theta
                if theta_min > theta:
            ...
            # Due to the fact that these lines are represented in polar form,
            # near vertical lines run the risk of jumping from an angle of 0 to
            # an angle of pi.  Should this occur, we instead compare the
            # supplementary angle.
    ...
    def process_frame(self, frame):
        '''Processes the input frame and looks for a path.
        
        Keyword Arguments:
        frame -- frame to process
        '''
        binary = self.prepare_frame(frame)
        lines = self.hough(binary)

        if self.find_path(lines):
        ...
        if self.debug:
            ...
            color_filtered = self.color_filtered
            # Show color filtered
    ...
    def __repr__(self):
        ...
        return "<PathTwoEntity center=%s theta=%s>" % \
            (self.center, theta)
